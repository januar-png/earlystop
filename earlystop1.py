...
class EarlyStop1:
    def __init__(self, model, config=None, early_stop_patience=10, min_delta=0, outdir="Model"):
        self.model = model
        self.min_delta = min_delta
        self.config = config
        self.early_stop_patience = early_stop_patience
        self.outdir = os.path.join("models", outdir)
    ...
    def early_stopping(self, model, monitor="test_score"):
        ...
        if self.counter >= self.early_stop_patience:
            print(f"Training early stopped. tidak ada peningkatan pada {self.early_stop_patience} epoch terakhir.")
